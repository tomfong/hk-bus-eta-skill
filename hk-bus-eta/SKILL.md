...
- **英文查詢 → 英文回覆**：當用戶用英文查詢時，必須用英文回覆，包括所有文字、站名、起點/終點資訊。
- **中文查詢 → 中文回覆**：當用戶用中文查詢時，用粵語風格中文回覆。
- 腳本輸出本身是中文，需要翻譯成英文（如適用）。
- 翻譯範例：
  -「往 將軍澳(寶琳)」→「To Tseung Kwan O (Po Lam)」
  -「機場 (地面運輸中心)」→「Airport (Ground Transportation Centre)」
  -「終點站」→「Terminus」
  -「分鐘後到」→「arriving in X minutes」
